...
export async function runCommand(
  cmd: string,
  args: string[],
  options: { root: string; cwd?: string; ignoreCmd?: string[]; onlyCmd?: string[]; importantCmd?: string[] }
): Promise<void> {
  ...
  const decision = decideRecording(commandLine, {
    ignoreCmd: options.ignoreCmd,
    onlyCmd: options.onlyCmd,
    importantCmd: options.importantCmd
  });
  let snapshot = decision.shouldRecordDiff
    ? await captureSnapshot(diffEngine, recorder, options.root, cwd)
    : null;
