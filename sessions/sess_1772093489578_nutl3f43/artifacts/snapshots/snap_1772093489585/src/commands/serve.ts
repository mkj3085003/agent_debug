...
export async function serveCommand(options: ServeOptions): Promise<void> {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    const sessionIdMatch = pathname.match(/^\/api\/sessions\/([^/]+)\/(restore|rerun)$/);
    if (req.method === "POST" && sessionIdMatch) {
      try {
        ...
        const engine = new ReplayEngine(rootDir);
        if (action === "restore") {
          const outDir =
            typeof body.outDir === "string" && body.outDir
              ? body.outDir
              : path.join(rootDir, "restores", `${sessionIdMatch[1]}_step_${step}`);
          await engine.restoreToStep(sessionIdMatch[1], step, outDir);
          sendJson(res, 200, { ok: true, outputDir: outDir });
          return;
        }
        const reuseOutputs = Boolean(body.reuseOutputs);
        await engine.rerun(sessionIdMatch[1], {
          mode: reuseOutputs ? "replay" : "rerun",
          fromStep: step,
          reuseOutputs
        });
        sendJson(res, 200, { ok: true });
      } catch (err) {
        const message = err instanceof Error ? err.message : "Failed to restore or rerun";
