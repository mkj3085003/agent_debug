...
program
  ...
  .option("--ignore-cmd <pattern>", "Skip diff for commands matching pattern (regex)", collect, [])
  .option("--only-cmd <pattern>", "Record diff only for commands matching pattern (regex)", collect, [])
  .option(
    "--important-cmd <pattern>",
    "Always record diff for commands matching pattern (regex)",
    collect,
    []
  )
  .action(runCommand);
